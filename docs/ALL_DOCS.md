...
> 本版同步更新：
> - 已完成接触窄相：面-面投影 + 一致切线近似 + 粘滑历史变量；
> - 已完成 S4/S4R、C3D8R 一致线性化积分接口与 hourglass 全量控制接口；
> - 已完成 J2 工程化基础：各向同性+随动硬化+温度屈服修正；
> - 已完成 PETSc 可选后端与 Schur 可配置混合（MPI 分布式主链下一阶段）。

### 推荐案例清单（覆盖结构/热/耦合/接触/塑性/官方风格）
...
3. 运行完整回归：`./scripts/run_regression.sh`

#### Windows CMD
1. 完成构建：`scripts\build_windows.bat`
2. 运行演示：`scripts\run_demo_windows.bat`
3. 运行完整回归：`scripts\run_regression.bat`

#### Windows PowerShell
- 推荐一键：`./scripts/deploy_windows.ps1`
...
3. 大规模模型可切换 PETSc 后端；MPI 分布式主链正在推进（下一阶段）。
