...
> 本版同步更新：
> - 接触窄相新增面-面投影与粘滑历史变量（工程化基础）；
> - 壳/实体新增 S4/S4R、C3D8R 多积分点与 hourglass 全量控制接口；
> - J2 返回映射新增各向同性+随动硬化及温度屈服修正；
> - 线性求解新增 PETSc 可选后端（需编译检测到 PETSc）。
...
3. 大规模模型可切换 PETSc 后端；MPI 分布式主链仍在推进。
